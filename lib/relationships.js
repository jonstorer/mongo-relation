...
Schema.prototype._addRelationship = function (type, model, options) {
  ...
  this.paths[pathName].options.setChild = true
  if(type == 'hasMany' && options)
    this.paths[pathName].options.setChild = options.hasOwnProperty('setChild') ? options.setChild : true;

  if (this.paths[pathName].options.setChild == false){
    if(this.paths[pathName].options.setParent == false)
      throw new Error('setChild and setParent cannot both be false');

    if (this.paths[pathName].options.dependent == 'nullify')
      throw new Error("dependent cannot be set to 'nullify' while setChild is false");

    if (this.paths[pathName].options.dependent == 'destroy')
...
MongooseArray.prototype.build = function (objs) {
  ...
  var setParent       = this._schema.options.setParent,
      setChild        = this._schema.options.setChild,
      self            = this,
      parent          = this._parent,
...
MongooseArray.prototype.find = function (conditions, fields, options, callback) {
  ...
  // TODO: check if there is a callback
  if (!this._hasRelationship())
    return callback(new Error('Path doesn\'t contain a reference'));
