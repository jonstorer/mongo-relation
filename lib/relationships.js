...
MongooseArray.prototype.build = function (objs) {
  ...
  var self            = this,
      ...
      setParent       = this._schema.options.setParent;
...
MongooseArray.prototype.find = function (conditions, fields, options, callback) {
  ...
  query = childModel.find(conditions).select(fields).where(childPath.name).equals(parent._id);
  if(setParent)
    query = query.where('_id').in(parent[this._path]);
...
MongooseArray.prototype.remove = MongooseArray.prototype.delete = function (id_or_child, callback) {
  var id,
      child,
...
MongooseArray.prototype.append = function (child, callback) {
  ...
  var relationship = this._getRelationship(),
      setParent    = this._schema.options.setParent,
      childPath    = child.schema._findPathReferencing(this._parent.constructor.modelName);
